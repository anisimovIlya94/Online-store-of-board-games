...
const CatalogSortRanges = ({title, rouble, initialState, data, onChange, name}) => {
    const [display, setDisaplay] = useState(true);
    // const [inputsValue, setInputsValue] = useState(data);
    ...
    return (
        <div style={{border: "1px solid #fff"}} className={"card my-2 " + classes.catalogSortWrapper}>
            <div className={"card-body " + classes.borderBottom}>
                ...
                <div className={"d-flex justify-content-center align-items-center my-4 " + classes.priceAllInputs}>
                         <label className={classes.label} htmlFor="">От</label>
                     <input className={classes.priceInput + " " + "mx-2"} onChange={handleChangeInputs} name="min" value={(minInitial === min) ? "" : min} placeholder={minInitial} type="text" />
                     <label className={classes.label} htmlFor="">До</label>
                     <input className={classes.priceInput + " " + "mx-2"} onChange={handleChangeInputs} name="max" value={(maxInitial === max) ? "" : max} placeholder={maxInitial} type="text" />
                     {rouble && <span className={classes.rouble}>₽</span>}
                    </div>
                    <div className={classes.range}>
                    <DoubleRangeSlider value={data} onChange={handleChangeValues} min={minInitial} max={maxInitial}/>
                    </div>
...
//                     <input className={classes.priceInput + " " + "mx-2"} type="text" />
