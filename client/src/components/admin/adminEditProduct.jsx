...
const AdminEditProduct = ({ prod }) => {
  ...
  const subcategoriesList = subcategories.map((category) => ({
    ...
    const defaultCategories = prod && prod.categories.map((category) => ({ value: category, label: getCategoryById(category).name }))
    const defaultSubcategories = prod && prod.subcategories.map((sub) => ({ value: sub, label: getSubCategoryById(sub).name }))
  ...
  const handleGoToProduct = () => {
    history.replace(`/catalog/${prod.categories[0]}/${prod.subcategories[0]}/${prod._id}`)
  }
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className={classes.formWrapper}>
        ...
        />
        {!deleteStatus && <div style={{ marginTop: "30px" }}>
          <ShoppingCardButton
          ...
          {prod && !deleteStatus && <ShoppingCardButton
      ...
      {prod && <button onClick={setDeleteQuestion} className="btn btn-danger rounded-pill mt-3 w-100">{deleteStatus ? "Удалено" : "Удалить товар"}</button>}
        {deleteState && <>
          <p style={{textAlign: "center", margin: "10px 0"}}>Вы уверены?</p>
          <div className="d-flex justify-content-center">
          <button onClick={handleDelete} className="btn btn-danger me-5">Да</button>
          <button onClick={setDeleteQuestion} className="btn btn-success">Нет</button>
          </div>
          </>}
    </div>
  );
